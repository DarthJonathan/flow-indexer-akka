...
import dev.lucasgrey.flow.indexer.FlowIndexerApplication.forceSyncBlockCmdHandler.handleForceSyncBlock
...
object BlockCommands {
  ...
  final case class ForceSyncBlock(block: FlowBlock, transactionList: List[FlowTransaction]) extends BlockCommand
  ...
  val blockCommandHandler: BlockCommandHandler = InspectCommandWrapper((state, cmd) => {
      cmd match {
        ...
        case cmd: ForceSyncBlock => handleForceSyncBlock(state, cmd)
